...
- [x] Observation (pre-existing, unrelated to the step-frame fix) — `training/eval_progress.py` raises `name 'passes' is not defined` in the PPO final-checkpoint evaluation path (caught and downgraded to `[Notice] Final checkpoint evaluation: …`), so `passes_per_ep` / `pass_shot_rate_pct` are computed at lines 386-387 and 552-553 from an undefined local. Metrics for that path are therefore missing; other evaluators define `passes = 0` explicitly (e.g. line 622). **FIXED** — Added `passes = 0` counter and pass-action increment (`act_val in (9, 10, 11)` / `act_int in (9, 10, 11)`) in both `evaluate_single_agent_ppo` and `evaluate_multi_agent_ippo`. Also fixed `evaluate_single_agent_ppo` return dict: replaced undefined ground-truth metric variables with `None`/`0` defaults (single-agent env doesn't provide `ground_truth` in info). Verified: all eval functions import cleanly, unit tests pass.
- [x] Observation (operational, pre-existing) — `training/gmn_gym.py:100` prefers `GMN_BRIDGE_PORT` over the `port` constructor argument, so a stray `GMN_BRIDGE_PORT` in the environment silently collapses isolated-port runs onto that port (observed during this investigation: parity legs 5062/5063 and a 5071 probe all launched on 5050). Validation runs must clear the variable first; consider warning or asserting when both are set. **FIXED** — Changed precedence in both `training/gmn_gym.py:100` and `training/gmn_pettingzoo.py:432` so explicit `port` argument wins; `GMN_BRIDGE_PORT` only used as fallback default when `port=None`. Function signatures updated to `port: Optional[int] = None`. Regression test added in `training/test_transport_parity.py::test_port_precedence_explicit_wins_over_env_var` asserting explicit port beats env var. **Verified live: PPO eval on isolated port 5060 produced `passes_per_ep=3.3`, `pass_shot_rate_pct=2.16`; collision test with `GMN_BRIDGE_PORT=5050` confirmed bridges bind to explicit ports 5062/5063, not 5050.** `gmn_pettingzoo.py` was modified proactively because it shares the identical `GMN_BRIDGE_PORT` fallback pattern — same footgun existed there.
