...
- [ ] P0.5c-observation (pre-existing, orthogonal to teardown) — `train_ppo.py` normal-path live runs hang on the first `step`: `[GMN-Gym WS Timeout] No frame received within 30.0s during 'step'` even with `GMN_WS_RECV_TIMEOUT=30` (bridge spawns, reset + model init succeed). Reproduced identically with the P0.5c edits stashed → not caused by the atexit change; teardown was clean in every variant (port free at T+0, no atexit errors). Root cause (engine/WS first-step behaviour on this machine) not yet diagnosed.
