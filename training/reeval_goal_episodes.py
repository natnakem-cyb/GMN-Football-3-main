"""
Re-evaluation script: 30-episode deterministic eval of the unchanged 200k MAPPO checkpoint
against the corrected RuleBasedAgent goalkeeper logic.
"""
import os
import sys
import json
...
ACTION_NAMES = {
    0: "action_idle", 1: "action_left", 2: "action_top_left", 3: "action_top", 4: "action_top_right",
    5: "action_right", 6: "action_bottom_right", 7: "action_bottom", 8: "action_bottom_left",
    9: "action_long_pass", 10: "action_high_pass", 11: "action_short_pass", 12: "action_shot",
    13: "action_sprint", 14: "action_release_direction", 15: "action_release_sprint",
    16: "action_sliding", 17: "action_dribble", 18: "action_release_dribble",
}
...
def evaluate():
    ...
    print(f"=== RE-EVALUATION RESULTS (30 episodes, corrected keeper logic) ===")
    print(f"Goals scored: {len(goal_episodes)} / 30")
    print(f"Goal rate: {len(goal_episodes)/30*100:.1f}%")
    print(f"Total shots (action 12): {len(all_shots)}")
    print(f"Shots per episode: {len(all_shots)/30:.2f}")
    if all_speeds:
    ...
    out_path = "training/analysis_200k_goal_episodes_reeval.json"
    with open(out_path, "w") as f:
        json.dump({
            ...
            "goal_rate_pct": len(goal_episodes) / 30 * 100,
            "total_shots": len(all_shots),
            "shots_per_ep": len(all_shots) / 30,
    ...
    print(f"\nRe-evaluation results saved to: {out_path}")
